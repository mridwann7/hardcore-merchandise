...
const storeData = (bands) => {
    form.band = band.value ? band.value.id : bands[0].id;
    form.post(route("shirts.store"), {
        errorBag: "storeData",
        preserveScroll: true,
        onError: () => {
            if (form.errors.band) {
...
<template>
    <AppLayout title="Shirts">
        ...
        <div class="max-w-7xl mx-auto py-10 sm:px-6 lg:px-8">
            <FormSection @submitted="storeData(ramen_categories, ramen_stores)">
                <template #title>Add Shirt</template>

                <template #description>
                    Add a new shirt data to database.
                </template>
